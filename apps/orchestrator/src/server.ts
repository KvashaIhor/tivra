...
app.post('/api/build', async (req: Request, res: Response) => {
  ...
  if (missingKeys.length > 0) {
    res.status(400).json({
      error: `Missing required configuration: ${missingKeys.join(', ')}. Please make sure to provide your Anthropic API KEY and InsForge credentials in "PROVIDER CREDENTIALS" field.`,
      missingKeys,
    });
    return;
  }
